...
// grpc.Dial("etcd:///user-service/v1", ... ->
// target.Scheme → "etcd"
// target.Endpoint → "my-service"
...
func (r *Resolver) Build(target resolver.Target,
	...
	serviceName := parts[0] // "user-service"
	version := ""
	if len(parts) > 1 {
		version = parts[1] // "v1"
	}

	// exp: /user-service/v1/(need prefix only)
	r.keyPrefix = BuildPrefix(ServerNode{
		Name:    serviceName, // "user-service/v1" -> target.Endpoint
		Version: version})    // "/user-service/v1" -> target.URL.Path
...
func (r *Resolver) watch() {
	...
	// init watchCh
	r.watchCh = r.cli.Watch(context.Background(),
		r.keyPrefix, clientv3.WithPrefix())
